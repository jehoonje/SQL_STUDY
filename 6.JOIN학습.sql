...
-- 회원정보(user)와 회원의 주문내역(order)를 조회
...
SELECT 
        E.emp_nm, 
        D.dept_nm
FROM tb_emp E -- 왼쪽은 다보여줘 -- 오른쪽은 ON에 매칭되는 거만보여줘
...
SELECT 
        *
FROM tb_emp E -- 왼쪽은 다보여줘 -- 오른쪽은 ON에 매칭되는 거만보여줘
LEFT JOIN tb_dept D -- OUTER조인은 매칭이 안되는 쪽은다 NULL로 표시
ON E.dept_cd = D.dept_cd
;
...
WHERE E.dept_cd = D.dept_cd (+) -- LEFT OUTER JOIN
;
SELECT 
        E.emp_nm, 
        D.dept_nm
FROM tb_emp E , tb_dept D 
WHERE E.dept_cd (+) = D.dept_cd -- RIGHT OUTER JOIN
;
-- # RIGHT OUTER JOIN
...
ON E.dept_cd (+) = D.dept_cd -- RIGHT OUTER JOIN
;
...
SELECT 
        ...
        D.dept_nm
FROM tb_emp E ,tb_dept D 
WHERE E.dept_cd (+) = D.dept_cd -- 오라클 조인
;
...
-- OUTER 조인은 INNER의 결과는 다나오고 추가로 뭐가더나옴
-- 사원이 46명, 부서가 16개
-- 부서에 소속된 사원이 41명
-- 사원이 존재하는 부서 14개
-- INNER JOIN의 결과 41건
-- LEFT JOIN : INNER 41 + 부서가없는 사원 5건 = 46건
