...
INSERT INTO char_compare VALUES ('103', 'SQLD2', '  SQLD');
...
-- 단일행 함수
SELECT 
        emp_nm, SUBSTR( addr, 1, 3 )  -- 문자를 자르는 함수
FROM tb_emp
;
...
-- 날짜 변환 함수
SELECT
        TO_DATE('20150101000000', 'YYYYMMDDHH24MISS') AS "날짜",
        TO_DATE('20240101', 'YYYYMMDD') AS "날짜 2",
        TO_DATE('202401', 'YYYYMM') AS "날짜 3",
        TO_CHAR(SYSDATE, 'YYYYMM') AS "날짜 4"
FROM dual;        
...
SELECT
        coalesce(null,  'A'), -- null 이 아닌 첫번째 값 
        NULLIF('A', 'A')  -- 두개 같으면 null
FROM dual; 
