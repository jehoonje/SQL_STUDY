...
-- GROUP BY절에 사용한 컬럼만 SELECT에서 직접조회 가능
SELECT
        emp_no,
       COUNT(*)
FROM tb_emp
...
SELECT 
    emp_no, 
    TO_CHAR(SUM(pay_amt), 'L999,999,999') "사원별 총급여액",
    TO_CHAR(ROUND(AVG(pay_amt), 0), 'L999,999,999.99') "사원별 평균급여액",
    TO_CHAR(MAX(pay_amt), 'L999,999,999') "사원별 최고급여액",
    COUNT(pay_de) "급여 수령횟수"
FROM tb_sal_his
...
-- FROM -WHERE - GROUP BY - HAVING - SELET- ORDER BY 
-- 순서로 . 순서로 . 순서로 .
